...
impl ChainedPics {
    ...
    pub unsafe fn disable(&mut self) {
        self.pics[0].data.write(CMD_DISABLE);
        self.pics[1].data.write(CMD_DISABLE);
    }
    ...
    pub unsafe fn mask(&mut self, irq: u8) {
        ...
        let mask = self.pics[port].data.read();
        let mask = mask | (1 << irq);

        self.pics[port].data.write(mask);
    }
    ...
    pub unsafe fn maskall(&mut self) {
        self.pics[0].data.write(0xff);
        self.pics[1].data.write(0xff);
    }

    pub unsafe fn unmaskall(&mut self) {
